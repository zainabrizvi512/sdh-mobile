...
const GuideDetailView: React.FC<Props> = ({ guide, onBack, heroIcon, heroTint = GREEN }) => {
    ...
    return (
        <ScreenWrapper>
            ...
            <View style={styles.container}>
                ...
                {heroIcon ? (
                    <View style={[styles.heroBanner, { backgroundColor: heroTint + "17" }]}>
                        <View style={[styles.heroIconCircle, { backgroundColor: heroTint }]}>
                            <Ionicons name={heroIcon} size={34} color="#fff" />
                        </View>
                        <Text style={[styles.heroCaption, { color: heroTint }]}>{guide.disasterType?.name ?? "Safety Guide"}</Text>
                    </View>
                ) : null}
...
const styles = StyleSheet.create({
    ...
    heroBanner: { marginHorizontal: 20, marginTop: 16, borderRadius: 20, paddingVertical: 18, alignItems: 'center' },
    heroIconCircle: { width: 64, height: 64, borderRadius: 32, justifyContent: 'center', alignItems: 'center', elevation: 3 },
    heroCaption: { marginTop: 10, fontSize: 11, fontWeight: '800', letterSpacing: 0.6, textTransform: 'uppercase' },
